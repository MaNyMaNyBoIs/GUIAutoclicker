...
# bugs: esc hotkey only works when the autoclicker app window is in focus
# subscribe: https://bit.ly/BayMaxYT if you see this.



class Clicker():
    def __init__(self):
        self.DEFAULT_AMOUNT = 1000 # Default amount of clicks
        ...
        self.amount = tkinter.StringVar() # default number of clicks
        self.amount.set(self.DEFAULT_AMOUNT)
        self.speed = tkinter.StringVar() # default click interval in ms duhh
        self.speed.set(self.DEFAULT_SPEED)
    
        # Mainframe labels, entries, and buttons BASICALLY, THE GUI JEYSEUS KRIST
        self.amount_label = ttk.Label(self.mainframe, text = 'Number of clicks \n (Set to 0 for infinite clicks, until you stop the Autoclicker) \n (Hit the esc key or click on "Stop" to.. uhh stop (Window must be selected) )').grid(column = 1, row = 1, sticky = tkinter.W)
        self.speed_label = ttk.Label(self.mainframe, text = 'Click interval \n (in ms duh...)').grid(column = 1, row = 2, stick = tkinter.W)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.status == True:
                ...
                if self.entered_amount == 0 and t2 - t1 > self.entered_speed: # If the entered amount is 0, do infinite clicks
                    self.mouse_click()
                    t1 = time.clock()
                    ...
                    self.counter += 1
                    t1 = time.clock()
                elif self.counter >= self.entered_amount and t2 - t1 > self.entered_speed: # reset the counter once exceeded
                    self.state_false()

                self.mainframe.focus_set() #currently onlny works when Autoclicker window is in focus but needed to register esc key... 
